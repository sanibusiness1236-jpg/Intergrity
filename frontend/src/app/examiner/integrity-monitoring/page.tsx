...
interface Homophily { node: number; edge: number; adjusted: number; classH: number; unbiased: number; }
...
const NAV_TREE: NavSection[] = [
  ...
  { id: "results", label: "Prediction Results", icon: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2", items: [{ id: "results.all", label: "Predictions" }, { id: "results.logs", label: "Prediction Logs" }] },
  ...
  { id: "analyze", label: "Analyze Model", icon: "M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z", items: [] },
  { id: "reports", label: "Reports & Downloads", icon: "M12 10v6m0 0l-3-3m3 3l3-3M3 17V7a2 2 0 012-2h6l2 2h6a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2z", items: [] },
...
function computeHomophily(nodes: PredResult[], edges: [number, number][]): Homophily {
  const n = nodes.length;
  const E = edges.length;
  if (E === 0 || n === 0) return { node: 0, edge: 0, adjusted: 0, classH: 0, unbiased: 0 };

  // Edge homophily: fraction of edges connecting same-class nodes
  let sameEdges = 0;
  for (const [s, t] of edges) {
    if (nodes[s]?.prediction === nodes[t]?.prediction) sameEdges++;
  }
  const edgeH = sameEdges / E;

  // Node homophily: average per-node fraction of same-class neighbors
  const degrees = new Array(n).fill(0) as number[];
  const sameNbr = new Array(n).fill(0) as number[];
  for (const [s, t] of edges) {
    degrees[s]++; degrees[t]++;
    if (nodes[s]?.prediction === nodes[t]?.prediction) { sameNbr[s]++; sameNbr[t]++; }
  }
  let nodeH = 0, validN = 0;
  for (let i = 0; i < n; i++) { if (degrees[i] > 0) { nodeH += sameNbr[i] / degrees[i]; validN++; } }
  nodeH = validN > 0 ? nodeH / validN : 0;

  // Adjusted homophily: accounts for class imbalance
  const classes = ["cheater", "honest"] as const;
  let sumSq = 0;
  for (const cls of classes) {
    const D_c = degrees.reduce((s, d, i) => s + (nodes[i]?.prediction === cls ? d : 0), 0);
    sumSq += Math.pow(D_c / (2 * E), 2);
  }
  const adjH = sumSq >= 1 ? 0 : (edgeH - sumSq) / (1 - sumSq);

  // Class homophily: per-class intra-class ratio averaged
  let classH = 0;
  for (const cls of classes) {
    let intra = 0, total = 0;
    for (const [s, t] of edges) {
      const sC = nodes[s]?.prediction === cls, tC = nodes[t]?.prediction === cls;
      if (sC || tC) total++;
      if (sC && tC) intra++;
    }
    if (total > 0) classH += intra / total;
  }
  classH /= classes.length;

  return { node: nodeH, edge: edgeH, adjusted: adjH, classH, unbiased: adjH };
}
...
function computeLayout(nodes: PredResult[], edges: [number, number][], layout: Layout, W: number, H: number): { x: number; y: number }[] {
  ...
  for (let it = 0; it < iters; it++) {
    const fx = new Array(n).fill(0) as number[], fy = new Array(n).fill(0) as number[];
...
export default function IntegrityMonitoringPage() {
  ...
  const [selectedGraphModel, setSelectedGraphModel] = useState("vanilla_gcn");
  ...
  async function loadActivity() {
    ...
    setLoadingActivity(true); setPredMsg("");
    try {
      const { data } = await api.get(`/integrity/exam/${selectedExamId}/activity`);
      setActivityRows(data.data.rows); setActivityExam(data.data.exam);
    } catch (e: any) { setPredMsg(e.response?.data?.error?.message || "Failed to load activity data"); }
  ...
  async function runPrediction() {
    ...
    try {
      ...
      setPredResult(data.data); nav("results.all");
      setPredMsg(`Prediction complete — ${data.data.cheaters} cheater(s) detected.`);
    } catch (e: any) { setPredMsg(e.response?.data?.error?.message || "Prediction failed"); }
  ...
  async function runUploadedPredict() {
    ...
    try {
      ...
      const mapped: PredResponse = {
        ...
        high_risk: fake.predictions?.filter((p: any) => p.flagged_prob >= 0.7).length ?? 0,
        medium_risk: fake.predictions?.filter((p: any) => p.flagged_prob >= 0.3 && p.flagged_prob < 0.7).length ?? 0,
        low_risk: fake.predictions?.filter((p: any) => p.flagged_prob < 0.3).length ?? 0,
        results: (fake.predictions || []).map((p: any) => ({
          student_id: p.student_id, student_name: p.student_id, student_username: p.student_id,
          prediction: (p.prediction === "flagged" ? "cheater" : "honest") as "cheater" | "honest",
          label: (p.prediction === "flagged" ? "Cheater" : "Honest") as "Cheater" | "Honest",
          risk: (p.flagged_prob >= 0.7 ? "high" : p.flagged_prob >= 0.3 ? "medium" : "low") as "high" | "medium" | "low",
  ...
  async function loadBenchmark() {
    ...
    try { const { data } = await api.get("/integrity/evaluate/all"); setBenchmark(data.data); }
  ...
  const graphEdges = useMemo((): [number, number][] => {
    ...
    for (let i = 0; i < r.length; i++) for (let j = i + 1; j < r.length; j++) {
      const a = r[i], b = r[j];
      const shared = Number(a.features.tab_switch_flag && b.features.tab_switch_flag) + Number(a.features.answer_paste_flag && b.features.answer_paste_flag) + Number(a.features.usb_detected && b.features.usb_detected) + Number(a.features.multi_device_login && b.features.multi_device_login);
      if (a.flagged_prob > 0.3 && b.flagged_prob > 0.3 && shared >= 1) edges.push([i, j]);
    }
    return edges;
  }, [predResult]);
  ...
  function downloadPNG() {
    ...
    const img = new Image();
    img.onload = () => {
      ctx.fillStyle = "#0f172a"; ctx.fillRect(0, 0, GW, GH); ctx.drawImage(img, 0, 0);
      const a = document.createElement("a"); a.href = canvas.toDataURL("image/png"); a.download = "gnn_graph.png"; a.click();
    };
  ...
  function renderContent() {
    ...
    if (sec === "results") return predResult ? <ResultsSection predResult={predResult} sub={sub} /> : <Empty msg="No prediction results yet. Run a prediction from Submit Scores." />;
    if (sec === "graph") return predResult ? <GraphSection nodes={predResult.results} edges={graphEdges} positions={graphPositions} layout={graphLayout} setLayout={setGraphLayout} svgRef={svgRef} selectedNodeIdx={selectedNodeIdx} setSelectedNodeIdx={setSelectedNodeIdx} onDownloadSVG={downloadSVG} onDownloadPNG={downloadPNG} sub={sub} GW={GW} GH={GH} selectedModel={selectedGraphModel} setSelectedModel={setSelectedGraphModel} /> : <Empty msg="No prediction data. Run a prediction first." />;
    ...
    if (sec === "settings") return <SettingsSection selectedModel={selectedModel} setSelectedModel={setSelectedModel} graphLayout={graphLayout} setGraphLayout={setGraphLayout} sub={sub} />;
    return null;
  }
        ...
        <aside className="w-full lg:w-56 lg:shrink-0">
          <div className="lg:sticky lg:top-6 space-y-0.5 rounded-xl border border-white/5 bg-slate-950/60 p-2 backdrop-blur-xl">
            {NAV_TREE.map((sec) => {
              ...
              const isActive = hasItems ? activeItem.startsWith(sec.id + ".") : activeItem === sec.id;
              return (
                <div key={sec.id}>
                  <button
                    onClick={() => {
                      if (!hasItems) { nav(sec.id); }
                      else { toggleSection(sec.id); if (!isExp) nav(sec.items[0].id); }
                    }}
                    className={`flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-[11px] font-semibold transition ${isActive ? "bg-indigo-500/15 text-indigo-300" : "text-white/55 hover:bg-white/5 hover:text-white"}`}
                  >
                  ...
                  {isExp && hasItems && (
                    <div className="ml-2 mt-0.5 space-y-0.5 border-l border-white/5 pl-3">
                      {sec.items.map((item) => (
                        <button key={item.id} onClick={() => nav(item.id)} className={`block w-full rounded-md px-2 py-1.5 text-left text-[10px] transition ${activeItem === item.id ? "bg-white/10 font-semibold text-white" : "text-white/35 hover:bg-white/5 hover:text-white"}`}>{item.label}</button>
                      ))}
                    </div>
                  )}
                </div>
...
function OverviewSection({ overview, sub }: { overview: Overview | null; sub: string }) {
  ...
  const items = [
    ...
    { label: "Predictions Run", value: overview.totalPredictions, accent: "purple" as const, icon: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" },
  ];
  return (
    <div className="space-y-6">
      <SectionHeader title={sub === "summary" ? "Integrity Summary" : sub === "sessions" ? "Active Monitoring Sessions" : sub === "submissions" ? "Total Submissions" : "Overview"} />
...
function CourseSection({ exams, selectedExamId, setSelectedExamId, activityRows, activityExam, loading, onLoad, sub }: { exams: Exam[]; selectedExamId: string; setSelectedExamId: (v: string) => void; activityRows: ActivityRow[] | null; activityExam: { title: string; courseCode: string } | null; loading: boolean; onLoad: () => void; sub: string; }) {
  const title = sub === "logs" ? "Student Activity Logs" : sub === "count" ? "Submission Count" : sub === "history" ? "Monitoring History" : "Course Prediction Sessions";
  return (
    <div className="space-y-6">
      ...
      {activityRows && !loading && (
        <GlowCard title={`Activity Logs — ${activityExam?.title}`} description={`${activityRows.length} student session(s)`}>
            ...
            <div className="max-h-80 overflow-auto rounded-lg border border-white/10">
              <table className="w-full text-xs">
                <thead className="sticky top-0 bg-slate-950/90">
                  <tr>
                    {["Student Username", "Student Name", "tab_switch_flag", "tab_switch_count", "time_away_exam_site", "answer_paste_flag", "usb_device_detection_count", "window_minimize_flag", "multi_device_login_flag"].map((h) => (
                      <th key={h} className="border-b border-white/10 p-2.5 text-left font-semibold uppercase tracking-wider text-white/40">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {activityRows.map((r) => (
                    <tr key={r.student_id} className="border-b border-white/5 hover:bg-white/[0.03]">
                      <td className="p-2.5 font-medium text-white">{r.student_username}</td>
                      <td className="p-2.5 text-white/70">{r.student_name}</td>
                      <td className="p-2.5">{r.tab_switch_flag ? <span className="text-rose-400 font-semibold">Yes</span> : <span className="text-white/30">No</span>}</td>
                      ...
                      <td className="p-2.5 text-white/50">{r.time_away_exam_site}</td>
                      <td className="p-2.5">{r.answer_paste_flag ? <span className="text-rose-400 font-semibold">Yes</span> : <span className="text-white/30">No</span>}</td>
                      <td className="p-2.5">{r.usb_device_detection_count > 0 ? <span className="text-rose-400 font-semibold">{r.usb_device_detection_count}</span> : <span className="text-white/30">0</span>}</td>
                      <td className="p-2.5">{r.window_minimize_flag ? <span className="text-rose-400 font-semibold">Yes</span> : <span className="text-white/30">No</span>}</td>
...
function SubmitSection({ exams, selectedExamId, setSelectedExamId, selectedModel, setSelectedModel, activityRows, activityExam, loadingActivity, onLoadActivity, predicting, onRunPredict, onCsvUpload, uploadedDatasetId, uploadName, uploading, onRunUploaded, msg, sub }: { exams: Exam[]; selectedExamId: string; setSelectedExamId: (v: string) => void; selectedModel: string; setSelectedModel: (v: string) => void; activityRows: ActivityRow[] | null; activityExam: any; loadingActivity: boolean; onLoadActivity: () => void; predicting: boolean; onRunPredict: () => void; onCsvUpload: (e: ChangeEvent<HTMLInputElement>) => void; uploadedDatasetId: string | null; uploadName: string; uploading: boolean; onRunUploaded: () => void; msg: string; sub: string; }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <GlowCard title="Step 1 — Generate Activity CSV" description="Pull behavioral signals from your exam sessions in the database">
        ...
        {activityRows && (
          <div className="rounded-lg border border-white/5 bg-white/[0.02] p-3 text-xs text-white/50">
            <span className="font-semibold text-white">{activityRows.length}</span> records loaded for <span className="font-semibold text-indigo-300">{activityExam?.title}</span>. Columns: student_username, student_name, tab_switch_flag, tab_switch_count, time_away_exam_site, answer_paste_flag, usb_device_detection_count, window_minimize_flag, multi_device_login_flag.
          </div>
        )}
      </GlowCard>
      ...
      <GlowCard title="Step 3 — Run Prediction on Exam Data" description="Use the activity data loaded above to run GNN prediction">
      ...
      <GlowCard title="Upload Custom CSV Dataset" description="Upload your own CSV (student_id + behavioral columns)">
        ...
        <p className="mt-3 text-xs text-white/30">Required: <code className="text-indigo-300">student_id</code>. Optional columns: tab_switch_count, paste_event_count, window_blur_count, usb_detected, multi_device_login, avg_answer_similarity, time_per_question_std, response_time_pattern, ip_similarity_score, seat_x, seat_y, label.</p>
      </GlowCard>
    </div>
  );
}

function ResultsSection({ predResult, sub }: { predResult: PredResponse; sub: string; }) {
  const FEATURE_COLS: { key: keyof typeof predResult.results[0]["features"]; label: string; binary: boolean }[] = [
  ...
  if (sub === "logs") {
    const distData = [{ name: "Cheater", count: predResult.cheaters, fill: "#f87171" }, { name: "Honest", count: predResult.honest, fill: "#34d399" }];
    return (
      <div className="space-y-6">
  ...
  return (
    <div className="space-y-4">
      <SectionHeader title="Predictions" desc={`${predResult.results.length} student(s) · Exam: ${predResult.exam.title} · Model: ${predResult.model_used}`} />
      <div className="grid grid-cols-3 gap-3 sm:grid-cols-3 mb-2">
        {[{ l: "Total", v: predResult.total, c: "text-white" }, { l: "Cheaters", v: predResult.cheaters, c: "text-rose-400" }, { l: "Honest", v: predResult.honest, c: "text-emerald-400" }].map((x) => (
      ...
      {predResult.results.length === 0 ? <Empty msg="No prediction results." /> : (
        <div className="overflow-x-auto rounded-xl border border-white/10">
          <table className="w-full text-xs">
            <thead className="sticky top-0 bg-slate-950/90">
              <tr>
                {["Student Name", "Username", "Prediction", "Confidence", ...FEATURE_COLS.map((f) => f.label)].map((h) => (
                  <th key={h} className="border-b border-white/10 p-2.5 text-left font-semibold uppercase tracking-wider text-white/35">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {predResult.results.map((r) => (
...
function GraphSection({ nodes, edges, positions, layout, setLayout, svgRef, selectedNodeIdx, setSelectedNodeIdx, onDownloadSVG, onDownloadPNG, sub, GW, GH, selectedModel, setSelectedModel }: { nodes: PredResult[]; edges: [number, number][]; positions: { x: number; y: number }[]; layout: Layout; setLayout: (v: Layout) => void; svgRef: RefObject<SVGSVGElement | null>; selectedNodeIdx: number | null; setSelectedNodeIdx: (v: number | null) => void; onDownloadSVG: () => void; onDownloadPNG: () => void; sub: string; GW: number; GH: number; selectedModel: string; setSelectedModel: (v: string) => void; }) {
  const selNode = selectedNodeIdx !== null ? nodes[selectedNodeIdx] : null;
  const homophily = useMemo(() => computeHomophily(nodes, edges), [nodes, edges]);
  ...
  return (
    <div className="space-y-6">
      ...
      <GlowCard title="Graph Layout" description="Select how nodes are arranged">
      ...
      {nodes.length === 0 ? <Empty msg="No prediction data. Run a prediction first." /> : (
        <GlowCard>
          <div className="relative overflow-hidden rounded-xl">
            <svg ref={svgRef} width={GW} height={GH} className="w-full max-h-[520px]" style={{ background: "rgba(255,255,255,0.01)" }}>
              ...
              {nodes.map((nd, i) => {
                ...
                const isCheat = nd.prediction === "cheater";
                const r = 7 + nd.flagged_prob * 10;
                return (
          ...
          <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex gap-2">
              <GlowButton onClick={onDownloadSVG} variant="ghost" size="sm"><Svg d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" size={13} /> SVG</GlowButton>
              <GlowButton onClick={onDownloadPNG} variant="ghost" size="sm"><Svg d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" size={13} /> PNG</GlowButton>
            </div>
          </div>
        </GlowCard>
      )}
      ...
      {nodes.length > 0 && edges.length > 0 && (
        <GlowCard title="Graph Homophily Analysis" description="Measures how often connected nodes share the same class (Honest / Cheater). Higher = more clustered by class.">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
            {[
              { label: "Node Homophily", value: homophily.node, desc: "Avg per-node same-class neighbor ratio" },
              { label: "Edge Homophily", value: homophily.edge, desc: "Fraction of same-class edges" },
              { label: "Adjusted Homophily", value: homophily.adjusted, desc: "Corrected for class imbalance" },
              { label: "Class Homophily", value: homophily.classH, desc: "Per-class intra-class ratio avg" },
              { label: "Unbiased Homophily", value: homophily.unbiased, desc: "Bias-corrected adjusted measure" },
            ].map((h) => (
              <div key={h.label} className="rounded-xl border border-white/5 bg-white/[0.02] p-3 text-center">
                <p className="text-[9px] uppercase tracking-wider text-white/30 mb-1">{h.label}</p>
                <p className={`text-2xl font-bold ${h.value > 0.6 ? "text-emerald-400" : h.value > 0.3 ? "text-amber-400" : "text-rose-400"}`}>{(h.value * 100).toFixed(1)}%</p>
                <p className="mt-1 text-[9px] text-white/20 leading-tight">{h.desc}</p>
              </div>
            ))}
          </div>
          <div className="mt-3 rounded-lg border border-white/5 bg-white/[0.02] p-3 text-xs text-white/40">
            <span className="font-semibold text-white/60">Interpretation: </span>
            {homophily.edge > 0.6 ? "High homophily — students cluster strongly by prediction class. Cheaters tend to connect with other cheaters." :
             homophily.edge > 0.3 ? "Moderate homophily — some clustering by class but also cross-class connections present." :
             "Low homophily — connections are largely between different classes. Consider reviewing edge threshold in Settings."}
          </div>
        </GlowCard>
      )}
      ...
      {selNode && (
        <GlowCard title={`Node: ${selNode.student_name}`} description={`${selNode.label} · ${selNode.risk} risk`}>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[{ l: "Prediction", v: selNode.label, c: selNode.prediction === "cheater" ? "text-rose-300" : "text-emerald-300" }, { l: "Confidence", v: `${(selNode.flagged_prob * 100).toFixed(1)}%`, c: "text-white" }, { l: "Risk Level", v: selNode.risk.toUpperCase(), c: selNode.risk === "high" ? "text-rose-300" : selNode.risk === "medium" ? "text-amber-300" : "text-emerald-300" }, { l: "Clean Prob", v: `${(selNode.clean_prob * 100).toFixed(1)}%`, c: "text-white" }].map((x) => (
...
function AnalyzeSection({ benchmark, loading, onLoad, sub }: { benchmark: BenchmarkResult | null; loading: boolean; onLoad: () => void; sub: string; }) {
  return (
    <div className="space-y-6">
      <SectionHeader title="Analyze Model" desc="F1, Precision, Recall, Accuracy, Confusion Matrix — evaluated on synthetic benchmark data with known labels" />
      {!benchmark && !loading && (
        <GlowCard title="Run Model Benchmark" description="Evaluate all GNN models on test data with known labels">
              ...
              <GlowCard key={r.model} title={r.model.replace(/_/g, " ").toUpperCase()}>
                ...
                {cm && (
                  <div>
                    ...
                    {FP > 0 && <p className="mt-2 rounded border border-amber-500/20 bg-amber-500/5 p-2 text-xs text-amber-300">⚠ {FP} honest student(s) incorrectly flagged as cheaters (False Positive).</p>}
                    {FN > 0 && <p className="mt-2 rounded border border-rose-500/20 bg-rose-500/5 p-2 text-xs text-rose-300">⚠ {FN} cheater(s) missed by the model (False Negative).</p>}
                  </div>
                )}
              </GlowCard>
            );
      ...
      <GlowCard title="Processing Status">
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-3 rounded-lg border border-white/5 bg-white/[0.02] p-3">
            ...
            <p className="text-sm text-white">{uploadedDatasetId ? `Dataset loaded — ID: ${uploadedDatasetId}` : "No dataset uploaded"}</p>
          </div>
        </div>
...
function SettingsSection({ selectedModel, setSelectedModel, graphLayout, setGraphLayout, sub }: { selectedModel: string; setSelectedModel: (v: string) => void; graphLayout: Layout; setGraphLayout: (v: Layout) => void; sub: string; }) {
  return (
    <div className="space-y-6">
      <SectionHeader title="Settings" />
      <GlowCard title="Default Model" description="The GNN model used by default when running predictions">
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
      ...
      <GlowCard title="Default Graph Layout" description="Layout algorithm used when first rendering the GNN graph">
        <div className="flex flex-wrap gap-2">
