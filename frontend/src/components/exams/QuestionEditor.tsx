...
export function questionToForm(q: Partial<Question>): QuestionFormValue {
  return {
    ...
    correctAnswerStr: q.type === "MCQ" || q.type === "TRUE_FALSE" || q.type === "FILL_IN_BLANK"
      ? String(q.correctAnswer ?? "")
      : "",
    blanks: q.type === "MULTI_BLANK_EQUATION" && Array.isArray(q.correctAnswer)
...
export function formToPayload(form: QuestionFormValue): { payload: Partial<Question> | null; error: string | null } {
  if (!form.text.trim()) return { payload: null, error: "Question text is required" };
  if (form.marks < 1) return { payload: null, error: "Marks must be at least 1" };
  ...
  if (form.type === "MCQ") {
    ...
    return { payload: { type: "MCQ", text: form.text, options: opts, correctAnswer: form.correctAnswerStr, marks: form.marks }, error: null };
  }
  ...
  if (form.type === "TRUE_FALSE") {
    ...
    return { payload: { type: "TRUE_FALSE", text: form.text, correctAnswer: form.correctAnswerStr, marks: form.marks }, error: null };
  }

  if (form.type === "FILL_IN_BLANK") {
    if (!form.correctAnswerStr.trim()) return { payload: null, error: "Provide the expected answer" };
    return { payload: { type: "FILL_IN_BLANK", text: form.text, correctAnswer: form.correctAnswerStr.trim(), marks: form.marks }, error: null };
  }
  ...
  if (form.type === "MULTI_BLANK_EQUATION") {
    ...
    if (blanksInText !== blanks.length) {
      return { payload: null, error: `Text has ${blanksInText} "___" but ${blanks.length} answer(s) provided` };
    }
    return { payload: { type: "MULTI_BLANK_EQUATION", text: form.text, correctAnswer: blanks, marks: form.marks }, error: null };
  }
...
export function QuestionEditor({ initial, onSave, onCancel, saveLabel = "Save", lockedType = false }: QuestionEditorProps) {
  ...
  function changeType(t: QuestionType) {
    setForm({ ...form, type: t, correctAnswerStr: "" });
  }
      ...
      <div className="space-y-1.5">
        <label className="text-[10px] font-semibold uppercase tracking-wider text-white/50">
          ...
          {form.type === "MULTI_BLANK_EQUATION" && (
            <span className="ml-2 normal-case text-white/40">— use <code className="rounded bg-white/10 px-1">___</code> for each blank</span>
          )}
        </label>
        <textarea
          ...
          placeholder={form.type === "MULTI_BLANK_EQUATION" ? 'e.g. "SELECT ___ FROM users WHERE id = ___"' : "Enter the question..."}
          required
        />
      </div>
        ...
        <div className="space-y-2">
          ...
          {form.options.map((opt, i) => (
            <div key={i} className="flex items-center gap-2">
              ...
              >
                {opt && form.correctAnswerStr === opt && <span className="h-2 w-2 rounded-full bg-emerald-400" />}
              </button>
